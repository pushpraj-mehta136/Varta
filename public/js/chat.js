...
// === RENDER MESSAGE ===
function addMessage({ from, message, timestamp, type, originalName, reactions = {} }, status = "sent") {
  const container = document.getElementById("messages");
  ...
  for (const [user, emoji] of Object.entries(reactions)) {
    if (emoji) {
      reactionBox.dataset[user] = emoji;
    }
  }

  renderReactions(reactionBox);
  ...
  // ✅ CRITICAL FIX: Ensure correct target user passed to reaction module
  const toUser = from === myUsername ? currentChat : from;
  addReactionButton(bubble, time, timestamp, from === myUsername, toUser);

  // === Track for future status updates ===
  if (from === myUsername) {
    messageStatusMap[timestamp] = time;
  }
}

// === RENDER REACTIONS (Helper) ===
function renderReactions(box) {
  if (!box) return;
  box.innerHTML = ""; // Clear old reactions

  const emojis = Object.values(box.dataset);
...
// === UPDATE PARTNER STATUS (typing / last seen) ===
